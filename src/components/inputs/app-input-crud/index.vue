...
draggable(
  ...
  label.input__contato(:for="`GET-${name}-` + key" v-for="(item, key, index) in user[value]")
    .right
      button.btn.delete.bullet.small.left(
        type="button"
        @click="remove(value, key)"
        :aria-label="$t(`aria-label.${name}_remove`) + ' ' + user[value][key]"
      )
        i -
      button.btn.plus.bullet.small.left.ml-1(
        type="button"
        @click="newInput(value, vuex)"
        :aria-label="$t(`aria-label.${name}_add_new`)"
      )
        i +
      button.handle.btn.bullet.small.left.ml-1(
        type="button"
        :aria-label="$t('aria-label.move') + ' ' + user[value][key]"
      )
        i =
