...
form#curriculo(:class="{ renderActive: user.name}")
  slot
  // Box Personal
  box(title="true" type="personal" translate="h1")
    .row
    ...
    .row
      text-area-app(name="about" value="about" translate="about" vuex="updateAbout")
    .row
      select-hobby(:errors="errors")
  // Box Telephone
  ...
  box(v-if="user.sites.length == 0")
    button#newSite.btn.plus.large(type="button" @click="newInput('sites', 'updateSite')" :aria-label="$t('aria-label.site_add')")
      ...
      | {{ $t('form.site') }}
  // Box Language
  box(v-if="user.languages.length !== 0")
    input-crud(name="language" value="languages" type="text" translate="language" vuex="updateLanguage" language="true" :errors="errors")
  box(v-if="user.languages.length == 0")
    button#newLanguage.btn.plus.large(type="button" @click="newInput('languages', 'updateLanguage')" :aria-label="$t('aria-label.language_add')")
      ...
      | {{ $tc('form.language', 1) }}
  // Box Address
  box(title="true" type="address" translate="end")
    .row
...
import NotificationAlert from '@/components/lab/NotificationAlert'
