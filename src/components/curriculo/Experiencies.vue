<template lang="pug">
  .box.experiences
    button.btn.plus.large(type="button" v-if="user.exps.length == 0" @click='newComponent()' aria-label="Add experience")
      span +
      | {{ $t('form.exp')}}
    fieldset.experience(v-for="(exp, key, index) in user.exps" :id="'exp-' + exp.id" :key="key")
      legend
        ...
        .right.ml-1
          button.btn.delete.bullet.small.left(type="button" @click="removeComponent(key)" aria-label="Remove experience")
            i -
          button.btn.plus.bullet.small.left(type="button" @click="newComponent(key)" v-scroll-to="'#exp-' + exp.id_sibiling"  aria-label="Add new experience")
            i +
      // Experiência atual?
      // The exps Now?
