<template lang="pug">
  .box.coursers
    button.btn.plus.large(type="button" v-if="user.coursers.length == 0" @click='newComponent()' aria-label="Add course")
      span +
      | {{ $t('form.cademy')}}
    fieldset.course(:id="'course-' + course.id" v-for="(course, key, index) in user.coursers" :key="key")
      legend
        ...
        .right.ml-1
          button.btn.delete.bullet.small.left(type="button" @click='removeComponent(key)' aria-label="Remove course")
            i -
          button.btn.plus.bullet.small.left(type="button" @click='newComponent(key)' v-scroll-to="'#course-' + course.id_sibiling" aria-label="Add new course")
            i +
      // Curso atual?
      // The Course Now?
