...
form#curriculo(method="get" @submit="checkForm")
  fieldset
    ...
    label(for="GET-dataofbirth" v-bind:class="{ error: !user.age && errors.length }")
      p.text.text-left(v-if="!user.genero") {{ $t('form.born')}}
      p.text.text-left(v-if="user.genero && user.genero == 'h'") {{ $t('form.born_man')}}
      p.text.text-left(v-if="user.genero && user.genero == 'm'") {{ $t('form.born_woman')}}
    ...
    .address
      ...
      label(for="GET-address-number" v-bind:class="{ error: !user.addressNumber && errors.length }")
        p.text.text-left  {{ $t('form.number')}}
    ...
    div.box-travel.box(v-bind:class="{ 'display-n-print': !user.travel, error: !user.travel && errors.length }")
      p.text.text-left {{ $t('form.travel')}}:
    ...
    input#test-id.test-class(name="test-name" type="text" placeholder="Teste..." @input="updateCoreVuex($event)")
