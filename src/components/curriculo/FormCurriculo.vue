...
form#curriculo(:class="{ renderActive: user.name}")
  ...
  box-curriculo(v-if="user.sites.length == 0")
    button#newSite.btn.plus.large(type="button" @click="newInput('sites', 'updateSite')" :aria-label="$t('aria-label.site_add')")
      ...
      | {{ $t('form.site') }}
  // Box Language
  box-curriculo(v-if="user.languages.length !== 0")
    input-crud(name="language" value="languages" type="text" translate="language" vuex="updateLanguage" language="true" :errors="errors")
  box-curriculo(v-if="user.languages.length == 0")
    button#newSite.btn.plus.large(type="button" @click="newInput('languages', 'updateLanguage')" :aria-label="$t('aria-label.language_add')")
