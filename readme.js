...
for (const language in list_languages) {
  const name = `${list_languages[language][2]} - ${list_languages[language][0]}`
  const asset = `./src/assets/flags/${list_languages[language][1]}.svg`
  const url = `${urlHosting}/?lang=${list_languages[language][1]}`
